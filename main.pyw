...
# Fallback installer for missing dependencies
REQUIRED_LIBS = {
...
missing = [pip_name for mod_name, pip_name in REQUIRED_LIBS.items() if not __import__('importlib').util.find_spec(mod_name)]
if missing:
    ...
    for pkg in missing:
        tk.Label(box, text=f"• {pkg}", font=("Consolas", 10), fg="#34d399", bg="#1f2937").pack(anchor="w", padx=12, pady=2)

    def install_pkgs():
        root.destroy()
        subprocess.run(f'start cmd /c "{sys.executable} -m pip install {" ".join(missing)} && pause"', shell=True)
        subprocess.Popen([sys.executable.replace("python.exe", "pythonw.exe"), os.path.abspath(__file__)], creationflags=0x08000000 if sys.platform == "win32" else 0)
        sys.exit()

    btns = tk.Frame(root, bg="#111827")
    btns.pack(fill="x", padx=30, pady=10)
    tk.Button(btns, text="Cancel", font=("Segoe UI", 9), bg="#374151", fg="#fff", bd=0, padx=12, pady=5, command=sys.exit).pack(side="left")
    tk.Button(btns, text="Install Packages", font=("Segoe UI", 9, "bold"), bg="#059669", fg="#fff", bd=0, padx=12, pady=5, command=install_pkgs).pack(side="right")
...
class CodeUpdateDialog(ctk.CTkToplevel):
    def __init__(self, parent, target_file, on_success):
        ...
        ctk.CTkLabel(self, text="Paste updated code here:", font=("Segoe UI", 12, "bold"), text_color="#38bdf8").pack(anchor="w", padx=20, pady=(15, 5))
        self.editor = ctk.CTkTextbox(self, font=("Consolas", 11), fg_color="#020617", border_color="#1e293b", border_width=1)
        self.editor.pack(fill="both", expand=True, padx=20, pady=5)

        actions = ctk.CTkFrame(self, fg_color="transparent")
        actions.pack(fill="x", padx=20, pady=(10, 15))
        ctk.CTkButton(actions, text="Cancel", width=90, fg_color="#334155", command=self.destroy).pack(side="left")
        ctk.CTkButton(actions, text="Apply & Restart", width=140, fg_color="#059669", hover_color="#047857", command=self.apply).pack(side="right")
...
class ArchiveSuiteApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.btn_update = ctk.CTkButton(ctrls, text="Update", width=75, fg_color="#0369a1", hover_color="#0284c7", command=lambda: CodeUpdateDialog(self, os.path.abspath(__file__), self.restart_app))
